	...
	msharp.isNumber = function (object) {
		...
		return msharp.type(object).toLowerCase() === 'number';
	};
	...
	msharp.Type = (function () {

		/// <var type="Array">All possible values of the JavaScript typeof operator.</var>
		var PRIMITIVES = ['number', 'string', 'boolean', 'object', 'function', 'undefined'];

		// Constructor
		// -----------

		var Type = function (operand) {

			throw new NotImplementedError('msharp.Type');

			function getPrimitiveType(operand) {
				/// <summary>Returns a string representing one of the six JavaScript primitive types.</summary>
				/// <returns type="String" />

				return typeof operand;
			}

			function getClassType(operand) {
				/// <summary>Returns the name of the objects prototype. This is more precise than the primitive type name.</summary>
				/// <returns type="String" />

				return Object.prototype.toString.call(operand).match(/\[object (.+?)\]/i)[1];
			}

			function getConstructorType(operand) {
				/// <summary>Returns the name of the constructor that created the object, or the prototype type if a constructor name wasn't found.</summary>
				/// <returns type="String" />

				var prototypeType = getClassType(operand),
					match = null;

				if (prototypeType === 'Object') {
					if (operand.constructor) {
						if (operand.constructor.name) {
							return operand.constructor.name;
						} else {
							match = operand.constructor.toString().match(/^function(.+?)\(/i);
			...
			function isPrimitiveType(operand) {
				if (getPrimitiveType(operand) !== 'object') {
					return true;
				}
			...
			this.primitiveType = getPrimitiveType(operand);
			this.classType = getClassType(operand);
			this.contructorType = getConstructorType(operand);
			this.isPrimitiveType = isPrimitiveType(operand);
